...
// Colors (Pop-Punk Neon)
// Neon Blue, Neon Green, Hot Pink, Hot Orange, Electric Yellow
const BLUE   = { r: 0x00, g: 0xE5, b: 0xFF }; // #00E5FF
const GREEN  = { r: 0x39, g: 0xFF, b: 0x14 }; // #39FF14
const PINK   = { r: 0xFF, g: 0x2B, b: 0xD6 }; // #FF2BD6
const ORANGE = { r: 0xFF, g: 0x6A, b: 0x00 }; // #FF6A00
const YELLOW = { r: 0xFF, g: 0xFF, b: 0x33 }; // #FFFF33
const allColors = [BLUE, GREEN, PINK, ORANGE, YELLOW];
...
const touchTrailColor = 'rgba(253, 3, 149, 0.81)';
...
// Target model selector. Change to switch shapes without affecting gameplay.
// Options: 'cube-recursive' (default style), 'cube', 'octa'
const TARGET_MODEL_TYPE = 'octa';
// Billboard text targets: draw the lyric word at the target position.
const BILLBOARD_TEXT_TARGETS = true;
// If true, hide 3D meshes for targets so only text is visible.
const BILLBOARD_HIDE_MESH = true;
// Retro particles (pixel squares) instead of line sparks
const RETRO_PARTICLES = true;
// Billboard font family prioritizing retro pixel/terminal fonts
const BILLBOARD_FONT_FAMILY = "'Press Start 2P', 'VT323', 'Lucida Console', 'Courier New', monospace";

// Level / Progress
let levelDurationSec = 0; // set from audio metadata / JSON / lyrics fallback
let levelWon = false;

// Optional: Manually set the song title shown on the main menu.
// If non-empty, this will be used instead of meta.title/audioName.
const SONG_TITLE_OVERRIDE = 'PSALM 150';
...
// Minimal lyrics integration that loads an optional lyrics.json and
// displays the active word in a centered overlay. If an audio file is
// referenced by the JSON metadata, it will be created/played when the
// game starts so timing follows the track.

const lyricsNode = document.querySelector('.lyrics');
let lyricsWords = [];
let lyricIdx = 0;                 // overlay index
let nextLyricSpawnIdx = 0;        // spawn scheduler index
...
function setLyricsFromData(data) {
  try {
    ...
    const words = (data.mapped && Array.isArray(data.mapped.words)) ? data.mapped.words : [];
    lyricsWords = words.slice().sort((a, b) => a.t0 - b.t0);
    lyricIdx = 0;
    nextLyricSpawnIdx = 0;
    lyricsReady = lyricsWords.length > 0;
    ...
    if (audioSrc && !audioEl) {
      try {
        ...
        audioEl.addEventListener('loadedmetadata', () => {
          if (!levelDurationSec || !isFinite(levelDurationSec) || levelDurationSec < 1) {
            if (isFinite(audioEl.duration) && audioEl.duration > 1) {
              levelDurationSec = audioEl.duration;
            }
          }
        });
      } catch (_) {
    ...
    // Determine level duration from JSON meta or last word, as fallback
    if (!levelDurationSec || !isFinite(levelDurationSec) || levelDurationSec < 1) {
      const metaDur = Number(meta.duration);
      if (isFinite(metaDur) && metaDur > 1) {
        levelDurationSec = metaDur;
      } else if (lyricsWords.length) {
        const last = lyricsWords[lyricsWords.length - 1];
        levelDurationSec = Math.max(1, Number(last.t1) || Number(last.t0) || 0);
      }
    }

    // Update Now Playing title if present (respect manual override)
    if (songTitleNode) {
      if (typeof SONG_TITLE_OVERRIDE === 'string' && SONG_TITLE_OVERRIDE.trim()) {
        songTitleNode.textContent = SONG_TITLE_OVERRIDE.trim();
      } else {
        let title = (meta.title && String(meta.title).trim()) || '';
        if (!title && meta.audioName) {
          title = String(meta.audioName).replace(/\.[^/.]+$/, '');
        }
        songTitleNode.textContent = title || songTitleNode.textContent || 'Psalm 150';
      }
    }
  } catch (_) {
    lyricsReady = false;
...
async function loadLyrics(url = 'lyrics.json') {
  try {
    const res = await fetch(url, { cache: 'no-store' });
    if (!res.ok) return; // silent if missing
    const data = await res.json();
    setLyricsFromData(data);
  } catch (_) {
    // ignore if not available (e.g., file:// or no lyrics provided)
  }
}

function getTimelineTimeSec() {
  // Do NOT offset lyrics timing; use raw audio time or game time
  if (audioEl && !isPaused()) {
    return Math.max(0, (audioEl.currentTime || 0));
  }
  return state.game.time / 1000;
}

function updateLyricsOverlayForTime(currentSec) {
  if (!lyricsNode) return;
  if (BILLBOARD_TEXT_TARGETS && lyricsReady) { // hide overlay if we render text as targets
    lyricsNode.textContent = '';
    return;
  }
  if (!lyricsReady || lyricsWords.length === 0) {
    lyricsNode.textContent = '';
    return;
  }
  // Advance index for elapsed words
  while (lyricIdx < lyricsWords.length && currentSec >= lyricsWords[lyricIdx].t1) {
    lyricIdx++;
  }
  const w = lyricsWords[lyricIdx];
  if (w && currentSec >= w.t0 && currentSec < w.t1) {
    lyricsNode.textContent = w.text || '';
  } else {
    lyricsNode.textContent = '';
  }
}

function updateLevelProgressUI(currentSec) {
  if (!levelBarFillNode || !levelTimeNode) return;
  const total = (levelDurationSec && isFinite(levelDurationSec)) ? levelDurationSec : 0;
  if (total > 0) {
    const pct = clamp(currentSec / total, 0, 1);
    levelBarFillNode.style.transform = `scaleX(${pct.toFixed(3)})`;
    const remaining = Math.max(0, total - currentSec);
    levelTimeNode.textContent = formatTime(remaining);
  } else {
    levelBarFillNode.style.transform = 'scaleX(0)';
    levelTimeNode.textContent = '--:--';
  }
}

function startLyricsPlaybackIfAny() {
  if (audioEl) {
    try {
      audioEl.currentTime = 0;
      audioEl.play();
    } catch (_) {
      // playback may be blocked until a user gesture; buttons trigger this.
    }
  }
}
...
const getTarget = (() => {
	...
	return function getTarget() {
		...
		// Target Parameters
		// --------------------------------
			let color = pickOne([BLUE, GREEN, ORANGE, YELLOW]);
		let wireframe = false;
		let health = 1;
		let maxHealth = 3;
		...
		target.isStrong = health > 1;
		target.isSlowmo = !!wireframe;
...
function addSpark(x, y, xD, yD) {
    ...
    if (RETRO_PARTICLES) {
        // Neon retro palette
        const palette = ['#00ffff', '#ff00ff', '#ffff00', '#00ff00', '#ff6600', '#ff4444', '#00aaff', '#ffcc00'];
        spark.color = pickOne(palette);
        spark.size = randomInt(4, 8);
    } else {
        spark.color = undefined;
        spark.size = undefined;
    }
...
const levelContainerNode = $('.level');
const levelTimeNode = $('.level__time');
const levelBarFillNode = $('.level__bar-fill');
...
function renderScoreHud() {
	...
	cubeCountNode.innerText = `LYRICS SLICED: ${state.game.cubeCount}`;
}
...
const scoreTitleNode = $('.score-title');
const songTitleNode = $('.song-title');
...
function renderMenus() {
	...
	switch (state.menus.active) {
        ...
        case MENU_SCORE:
            ...
            if (scoreTitleNode) {
                scoreTitleNode.textContent = levelWon ? 'Level Complete' : 'Game Over';
            }
            if (isNewHighScore()) {
...
// A simple octahedron made of 8 triangles.
function makeOctahedronModel({ scale = 1 }) {
    ...
    const vertices = [
        ...
        { x: -s, y:  0, z: 0 },  // 3 -X
        { x:  0, y:  0, z: -s }, // 4 -Z
        { x:  0, y: -s, z: 0 },  // 5 bottom
    ];
    const polys = [
        // top cap (winding to face outward)
        { vIndexes: [0, 1, 2] },
        { vIndexes: [0, 2, 3] },
        { vIndexes: [0, 3, 4] },
        { vIndexes: [0, 4, 1] },
...
// Factory to choose target model based on selector above
function makeTargetModel() {
    switch (TARGET_MODEL_TYPE) {
        case 'cube':
            return makeCubeModel({ scale: targetRadius });
        case 'octa':
            return makeOctahedronModel({ scale: targetRadius });
        case 'cube-recursive':
        default:
...
// Social / Share Actions (Pause + Score menus)
const INSTAGRAM_URL = 'https://www.instagram.com/christianaiband/';
const MUSIC_URL = 'https://linktr.ee/lostboyfound';
...
async function shareGame() {
    const title = 'Lyric Ninja';
    const song = (songTitleNode && songTitleNode.textContent || '').trim();
    const text = song ? `Playing ${song} in Lyric Ninja!` : 'Check out Lyric Ninja!';
...
document.querySelectorAll('.insta-btn').forEach(btn => handleClick(btn, () => openExternal(INSTAGRAM_URL)));
document.querySelectorAll('.music-btn').forEach(btn => handleClick(btn, () => openExternal(MUSIC_URL)));
document.querySelectorAll('.share-btn').forEach(btn => handleClick(btn, () => shareGame()));
...
function resetGame() {
    ...
    // Reset lyrics timeline to start
    lyricIdx = 0;
    nextLyricSpawnIdx = 0;
    if (audioEl) {
    ...
    if (bgVideoEl) {
        try { bgVideoEl.currentTime = 0; } catch (_) {}
    }
    levelWon = false;
    // Reset progress bar
    if (levelBarFillNode) levelBarFillNode.style.transform = 'scaleX(0)';
    if (levelTimeNode) levelTimeNode.textContent = '--:--';
}
...
// Spawn a single target using the standard spawn position/velocities
function spawnLyricTarget(centerX, centerY, labelText='') {
...
function tick(width, height, simTime, simSpeed, lag) {
    ...
    // Spawn targets
    if (lyricsReady) {
        // Lyrics-driven spawning: spawn a target for each word whose t0 has passed
        const currentSec = getTimelineTimeSec();
        while (nextLyricSpawnIdx < lyricsWords.length && lyricsWords[nextLyricSpawnIdx].t0 <= currentSec) {
            const label = lyricsWords[nextLyricSpawnIdx].text || '';
            spawnLyricTarget(centerX, centerY, label);
            nextLyricSpawnIdx++;
        }
    } else {
        // Default random spawning
        spawnTime -= simTime;
        if (spawnTime <= 0) {
            if (spawnExtra > 0) {
                spawnExtra--;
    ...
    targets.forEach(entity => {
        if (!(BILLBOARD_HIDE_MESH && lyricsReady)) {
...
function draw(ctx, width, height, viewScale) {
	...
	// 2D Sparks
	if (RETRO_PARTICLES) {
		// Render pixel squares with neon palette
		sparks.forEach(spark => {
			const lifeScale = (spark.life / spark.maxLife) ** 0.7;
			const size = Math.max(1, Math.round((spark.size || 6) * lifeScale));
			const sx = Math.round(spark.x);
			const sy = Math.round(spark.y);
			ctx.fillStyle = spark.color || '#00ffff';
	...
	// Billboard text for targets (draw after trails so it sits on top)
	if (BILLBOARD_TEXT_TARGETS && lyricsReady) {
		ctx.save();
		...
		targets.forEach(target => {
			...
			const focalLength = cameraDistance * sceneScale;
			const depthScale = focalLength / (cameraDistance - target.z);
			const fontPx = clamp(32 * depthScale, 16, 160);
			ctx.font = `${fontPx | 0}px ${BILLBOARD_FONT_FAMILY}`;
			// Outline for contrast
			ctx.lineWidth = Math.max(3, fontPx * 0.12);
			ctx.strokeStyle = 'rgba(0,0,0,0.55)';
			ctx.strokeText(label, Math.round(target.projected.x), Math.round(target.projected.y));
			// Fill
			const fill = target.isStrong ? '#FF2BD6' : (target.isSlowmo ? '#00E5FF' : '#FFFFFF');
			ctx.fillStyle = fill;
			ctx.fillText(label, Math.round(target.projected.x), Math.round(target.projected.y));
			// Small badge to indicate special types
			if (target.isStrong || target.isSlowmo) {
				const badge = target.isStrong ? 'x3' : 'S';
				const badgePx = Math.max(10, (fontPx * 0.5) | 0);
				ctx.font = `${badgePx}px ${BILLBOARD_FONT_FAMILY}`;
				ctx.lineWidth = Math.max(2, badgePx * 0.15);
				ctx.strokeStyle = 'rgba(0,0,0,0.55)';
				ctx.fillStyle = fill;
				const bx = Math.round(target.projected.x);
				const by = Math.round(target.projected.y - fontPx * 0.9);
				ctx.strokeText(badge, bx, by);
				ctx.fillText(badge, bx, by);
				// restore primary font for next label
				ctx.font = `${fontPx | 0}px ${BILLBOARD_FONT_FAMILY}`;
			}
		});
		ctx.restore();
...
function setupCanvas() {
	...
	function frameHandler(timestamp) {
		...
		// Update lyrics overlay from timeline (audio or game time)
		const timelineSec = getTimelineTimeSec();
		updateLyricsOverlayForTime(timelineSec);
		updateLevelProgressUI(timelineSec);
		// Level completion check
		if (!isMenuVisible() && levelDurationSec && timelineSec >= levelDurationSec) {
			winGame();
		}
	}
